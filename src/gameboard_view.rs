...
pub struct GameboardViewSettings {
    ...
    /// Edge radius around the 3x3 sections.
...
impl GameboardViewSettings {
    ...
    pub fn new() -> GameboardViewSettings {
        GameboardViewSettings {
            ...
            selected_background_color: [0.8, 0.8, 0.8, 1.0],
            text_color: [0.0, 0.0, 1.0, 1.0],
            not_editable_color: [1.0, 1.0, 1.0, 1.0],
        }
    }
}

/// Stores visual information about the gameboard.
...
impl GameboardView {
    ...
    pub fn draw<G: Graphics, C>(&self,
        ...
        if let Some(ind) = controller.selected_cell {
            ...
            Rectangle::new(settings.selected_background_color)
        ...
        for j in 0..9 {
            for i in 0..9 {
                if !controller.gameboard.is_editable([i, j]) {
                    let pos = [i as f64 * cell_size, j as f64 * cell_size];
                    let cell_rect = [
                        settings.position[0] + pos[0], settings.position[1] + pos[1],
                        cell_size, cell_size,
                    ];
                    Rectangle::new(settings.not_editable_color)
                                .draw(cell_rect, &c.draw_state, c.transform, g);
                }
